...
5. **AppImage FUSE namespace breaks WebKit on COSMIC / some Wayland compositors**: When the AppImage is launched normally, it FUSE-mounts itself into a private namespace. Inside that namespace, WebKitGTK's display / EGL detection picks the wrong backend and the WebView paints blank with 38% CPU on the web process. `pnpm tauri dev` is unaffected because it never enters the FUSE namespace. The user-visible repro is "dev works, AppImage is blank." The `lib.rs` env-var forcing block (`WEBKIT_DISABLE_DMABUF_RENDERER=1`, `GDK_BACKEND=x11`, `LIBGL_ALWAYS_SOFTWARE=1`) is now opt-in via `FXS_FORCE_SOFTWARE_GL=1` (default off) so we don't make the namespace detection worse. The actual fix when the AppImage is broken on a given machine is one of: (a) `./framexshot-x86_64.AppImage --appimage-extract-and-run` — extracts and runs in a plain temp dir, no FUSE namespace; (b) install the `.deb` from the same release; (c) `packaging/install.sh` from source. If a user reports "dev works, AppImage is blank" on a Wayland compositor, point them at the workarounds before debugging the code.
